...
### 修正（fork、2026-09-03、對照評估 data-deidentification 逼出）

- **統編檢核碼改財政部 112/4 新制**（`TwBusinessIdRecognizer._validate_checksum`）：
  加權乘積位數和由「可被 10 整除」改為「可被 5 整除」（新制為舊制超集、舊統編不受影響）；
  112/4 以後配發的統編約半數只符合新制、舊實作會判無效而**靜默漏遮**。
  連帶修正第 7 位為 7 的特例：官方規則是該項 2+8=10 可取 0 或 1（候選＝naive 和減 10 或減 9），
  舊實作寫成 `(total - 1) % 10`、會把財政部附件的官方合法範例 19312376 判無效；
  舊測試的三個「特例」統編是照錯實作反推的、已換成官方附件三例＋新制專屬例。
- **表單「標籤換行值」佈局的姓名漏網**（`TwLabeledNameRecognizer`）：PDF 文字層把表格
  每格各印一行（`委託人 \n林宜樺 \n`）、分隔集原只認 `|：:` → 整名殘留。加單一換行分隔形
  （空行不跨）；候選落選時從候選起點重掃、免「委託人\n姓名\n林宜樺」把第二個標籤吃掉。
- **姓氏補齊改「補到姓氏為止」**（`_expand_person_surname`）：CKIP 只標名末字時舊實作只補一字、
  輸出「姓＋名首字<PERSON>」（真件 27 處中 3 處）。改由大到小嘗試吸收 1–3 個漢字、
  單姓 ≤3 字／複姓 ≤4 字（k=1 一律放行、冠夫姓 4 字行為不變）；非漢字或他實體佔用即停。
